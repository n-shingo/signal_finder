...
# 更新 : 2019-11-08 SignalFinder2019の作成

1.概要
    つくばチャレンジ2019用信号検出プログラム
...
<開始信号>--> |                       |
    ...
    C. 青信号待ちのアルゴリズム
       ...
       3) 2)で相関係数が閾値未満出あった場合，8枚の青信号テンプレートとの相関係数を求める．最良の相関係
          数が閾値(=0.8)以上の場合は，青色信号候補画像とする．
       4) 青色信号候補画像が赤から青へ変化した画像であるか検証する.検証方法は差分画像のテンプレートマッチ
          ングと、上部が暗く下部が明るくなったかを確認する。検証できた場合は青信号を検出したと認識し、ア
          イドリング状態へ遷移する．
       5) 1)で領域候補がひとつもない，または，2)で赤信号候補がみつからず，3)，4)でも青信号が検出できなか
          った場合は、赤信号が検出できなかったとする.このことが N3(=10)フレーム連続で発生した場合は赤信
          号を見失ったとして，「B)赤信号検出」に遷移する．
